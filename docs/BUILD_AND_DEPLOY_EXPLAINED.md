...
说明日期：2026-09-16；内容依据本项目当前代码与工作流。
...
需要补充的条件是：托管服务还要配置好域名、HTTPS、目录首页和缓存等。因此，**日常内容更新主要是重新构建并上传文件；第一次上线还要做一次基础配置。**
...
另一个名为 browser-diagnostics 的 artifact 可能保存失败诊断材料，它不是网站发布文件。

**工作流文件存在，只说明配置了这些步骤；是否真的成功执行，要看具体一次运行的结果。** 本文解释当前配置，不把未检查的最新 GitHub 运行状态写成成功。
...
flowchart TD
    ...
    A -.->|"未来配置部署步骤"| O["阿里云 OSS：网站文件"]
    L -.->|"也可以手动部署"| O
    O --> D["CDN → 读者浏览器"]
```

图中的虚线表示可选或未来的部署路径，当前工作流尚未配置上传 OSS。
...
- 不会自动发布到 GitHub Pages、GitHub Releases 或阿里云。
...
未来配置部署后，上传工具会把构建产物再复制到 OSS Bucket。Bucket 可以理解为云上的一个文件存储容器。
...
GitHub 不知道你想把文件传到哪个 Bucket，也不能在没有授权的情况下写入你的云服务。因此，上传目标、权限和部署步骤要另外配置。
...
**当前 Actions 出现绿色勾，说明这一轮已配置的工作完成了；因为还没有部署步骤，所以它不代表网站已经更新。**
...
你不必每次手工安装整个构建环境、管理服务器进程、下载 artifact 再上传。把这些重复步骤自动化，正是这套链路最终想实现的效果。

### 在当前尚未配置自动部署时

前三步相同；之后只能得到经过检查的 artifact。网站更新还需要单独的手动或后续自动部署步骤。
...
不会仅仅因为构建失败而变化。如果还没有开始上传，线上仍然是上一份已部署内容。未来的部署步骤应在检查成功后才运行；上传过程中失败属于另一类问题，需要发布和回滚设计。
...
| 查看自动检查与构建步骤 | .github/workflows/ci.yml |
| 修改文章 | src/content/posts/ |
| 理解文章格式、图片、数学写法 | docs/CONTENT_GUIDE.md |
| 查看生成的网站成品 | 项目根目录 dist/ |
| 配置阿里云上线 | docs/DEPLOYMENT_GUIDE.md，以及工作区上一级的发布规划 |
